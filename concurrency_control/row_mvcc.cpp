...
void Row_mvcc::init(row_t * row) {
	...
	_requests = (ReqEntry *) mem_allocator.alloc(sizeof(ReqEntry) * _req_len, -1);
	...
	blatch = false;
	latch = (pthread_mutex_t *) mem_allocator.alloc(sizeof(pthread_mutex_t), -1);
...
Row_mvcc::double_list(uint32_t list)
{
	if (list == 0) {
		...
		assert(list == 1);
		ReqEntry * temp = (ReqEntry *) mem_allocator.alloc(sizeof(ReqEntry) * _req_len * 2, -1);
		for (uint32_t i = 0; i < _req_len; i++) {
		...
		for (uint32_t i = _req_len; i < _req_len * 2; i++)
			temp[i].valid = false;
		mem_allocator.free(_requests, sizeof(ReqEntry) * _req_len);
		_requests = temp;
		_req_len = _req_len * 2;
	}
}
